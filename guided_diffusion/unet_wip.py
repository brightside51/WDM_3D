    ...
    def forward(self, x, emb):
        for layer in self:
            if isinstance(layer, TimestepBlock):
                print(f"X_Seq      {x[0].shape}")
                print(f"Emb_Seq    {emb.shape}")
                x = layer(x, emb)
                print(f"X2_Seq     {x[0].shape}")
            else:
                print(f"X_Seq      {x[0].shape}")
                x = layer(x)
                print(f"X2_Seq      {x[0].shape}")
    ...
    def forward(self, x, timesteps = th.Tensor([1000]), y=None):
        ...
        for k, module in enumerate(self.output_blocks):
            ...
            if k >= self.decoder_device_thresh:
                ...
                #print(f"H New      {new_hs.shape}")

            if self.additive_skips:
                #print(f"H          {h.shape}")
                #print(f"H New      {new_hs.shape}")
                if h.shape[2:]!= new_hs.shape[2:]:
                    new_hs = F.interpolate(new_hs, size=h.shape[2:], mode='nearest')
                    #print(f"H Itp      {new_hs.shape}")
                h = (h + new_hs) / 2
            else:
                #print(f"H          {h.shape}")
