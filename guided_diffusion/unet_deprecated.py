...
class Downsample(nn.Module):
    ...
    def __init__(self, channels, use_conv, dims=2, out_channels=None, resample_2d=True):
        ...
        self.use_conv = use_conv
        self.dims = dims
        stride = (1, 2, 2)          # Only downsample height and width
        kernel_size = (1, 2, 2)     # Match the stride
...
class WaveletGatingDownsample(nn.Module):
    ...
    def forward(self, x, temb):
        ...
        if x.shape[2] < 64: x = F.pad(x, (0, 0, 0, 0, 0, 64 - x.shape[2]))
...
class UNetModel(nn.Module):
        ...
        self.input_blocks = nn.ModuleList(
            [
                TimestepEmbedSequential(
                    ...
                    conv_nd(dims, in_channels, model_channels, kernel_size=(3, 3, 3), padding=(1, 1, 1))
                )
            ]
        )
    ...
    def forward(self, x, timesteps = th.Tensor([1000]), y=None):
        """
        ...
        #x = x.to(self.devices)
        assert x.device == self.devices, f"{x.device=} does not match {self.devices=}"
        assert timesteps.device == self.devices, f"{timesteps.device=} does not match {self.devices=}"
        ...
        for k, module in enumerate(self.output_blocks):
            ...
            if k == self.decoder_device_thresh:
                h = h.to(self.devices)
                emb = emb.to(self.devices)
            if k >= self.decoder_device_thresh:
                new_hs = new_hs.to(self.devices)

            if self.additive_skips:
                #print(f"H1: {h.shape}")
                #print(f"New: {new_hs.shape}")
                h = (h + new_hs) / 2
                #print(f"H2: {h.shape}")
            else:
