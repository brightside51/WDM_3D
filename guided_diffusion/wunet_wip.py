...
class WaveletDownsample(nn.Module):
    ...
    def __init__(self, in_ch=None, out_ch=None):
        ...
        self.in_ch = in_ch
        self.out_ch = out_ch
        #self.stride = (1, 2, 2) if dims == 3 and resample_2d else 1
        self.conv = conv_nd(3, self.in_ch * 8, self.out_ch, 3, stride=1, padding=1)
        #self.conv = conv_nd(3, self.in_ch * 8, self.out_ch, 3, stride=stride, padding=1)
        self.dwt = DWT_3D('haar')
...
class WavUNetModel(nn.Module):
    ...
    def forward(self, x, timesteps = th.Tensor([1000] * 8)):
        """
